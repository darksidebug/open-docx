...
function resolvePdfFont(
  ...
  return PDFMAKE_FONTS.has(family) ? family : fallback;
}
...
async function convertBlockNode(node: TiptapNode, state: ConvertState, opts: BlockOpts): Promise<Content[]> {
  ...
  switch (type) {
    ...
    case "heading": {
      ...
      const alignment = alignmentFromAttrs(node.attrs);

      const content: ContentText & { width: string } = {
        text: children.length ? children : "",
        style: styleId,
        width: '*',
        ...(alignment ? { alignment } : {}),
    ...
    case "columnBlock": {
      ...
      const columnNodes = (node.content ?? []).filter(
        (n) => canonicalNodeType(n.type, state) === "column"
      );
...
async function convertTable(tableNode: TiptapNode, state: ConvertState): Promise<ContentTable> {
  ...
  for (const rowNode of rowNodes) {
    ...
    while (col < columnCount) {
      ...
      const cellDefaultAlignment = alignmentFromAttrs(pending.node.attrs);
      ...
      rowCells.push({
        ...
        // Apply alignment directly to the PDF cell.
        // This is especially important for table headers.
        alignment: cellDefaultAlignment,
...
function buildDocDefinition(doc: TiptapDocument, options: ConvertOptions): TDocumentDefinitions {
  const marginPt = (options.marginInches ?? 0.3) * 72;
